...
describe('Component OrderOption', () => {
  it('should render without crashing', () => {
    const component = shallow(<OrderOption name='Lorem ipsum' type='image'/>);
...
for(let type in optionTypes){
  describe(`Component OrderOption with type=${type}`, () => {
    ...
    switch (type) {
      ...
      case 'checkboxes' : {
        it('contains checkbox and options', () => {
          const select = renderedSubcomponent.find('.checkboxes');
          expect(select.length).toBe(1);
            
          const options = select.find('input').not('[value=""]');
          expect(options.length).toBe(mockProps.values.length);
          expect(options.at(0).prop('value')).toBe(mockProps.values[0].id);
          expect(options.at(1).prop('value')).toBe(mockProps.values[1].id);
        });
      ...
      case 'date': {
        it('contains DatePicker component', () => {
          const select = renderedSubcomponent.find('.component');
          expect(select.length).toBe(1);
